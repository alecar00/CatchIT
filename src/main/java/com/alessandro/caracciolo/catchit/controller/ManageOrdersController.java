...
public class ManageOrdersController {
    ...
    public List<OrderBean> discoverOrders(){
        ...
        for (Order rs: orders){
            OrderBean orderBean;
            if (rs.getRider() != null) {
                RiderBean rBean = new RiderBean(rs.getRider().getIdRider(), rs.getRider().getName());
                orderBean = new OrderBean(rs.getIdOrder(), rs.getAddress(), rs.getCostumer(), rs.getTelNumber(), rBean, rs.getTime(), rs.getStatus());
            } else {
                orderBean = new OrderBean(rs.getIdOrder(), rs.getAddress(), rs.getCostumer(), rs.getTelNumber(), rs.getTime(), rs.getStatus());
            }
